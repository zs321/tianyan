...
class PayController extends BaseController {
    ...
    public function codetime(){
        ...
        if(isset($data["BillNO"])){
            ...
            $codetime = M("order")->where("order_number = '$billNo'")->getField("codetime");

            if(!$codetime){
                $arr[0] = date("Y-m-d H:i:s");
            }else{
                $arr = json_decode($codetime,true);
                array_push($arr,date("Y-m-d H:i:s"));
            }

            M("order")->where("order_number = '$billNo'")->save(array("codetime"=>json_encode($arr)));
    ...
    public function qrcodePay($param = "",$type = ""){
        ...
        //keep_array($param,'abc.txt');

        //keep_array($param);
        /*$param = array (
          'ali_account' => '17172607760@163.com',
          'attach' => '',
          'body' => '购买商品-在线支付',
          'net_gate_url' => 'http://api6.1899pay.com:81',
          'notice_url' => 'http://api.1899pay.com//Services/txqpc/callback.aspx',
          'out_trade_no' => 'OD181122170449000',
          'return_url' => 'http://api.1899pay.com//Services/txqpc/jump.aspx',
          'sign' => '7402EF6CC2D4B297AE038BE929D9B5ED',
          'total_fee' => '0.02',
          'trade_type' => 'ALIPAYPC',
        ...
        $arr['type'] = $type;
        $local = '';   //判断二位码是否从本地数据库取出   是为1  不是为2   3账号离线   4手机挂掉
        $data_image = '';
        ...
        $sql = "select code_id,order_id from `p_order` where pay_status = 0 and FROM_UNIXTIME(addtime,'%Y:%m:%d %H:%i:%s') <= DATE_SUB(NOW(),INTERVAL $min MINUTE) ";
        ...
        if ($order['qrdecode'] && $order['pay_status']==0) {
            ...
            elseif($qrcode) {
                ...
                //判断今天帐号生成码  有没有达到20次
                /*$con['addtime'] =array(array("gt",strtotime(date("Y-m-d"))),array('lt',time()));//注意没有等于
                $count = M("qrcode_ali")->where($con)->getField('case when count(1) > 0 then 1 else 0 end as c');*/

                $accountShou = $param['ali_account'];
                $astatus = M("account")->where("account = '$accountShou'")->find();
                //keep_array($astatus);
                //0 个人 1企业
             if(!$astatus["astatus"]){
//                if(true $count &&){
                    //付款码
                    $qrcode = M("qrcode_ali")->where(["account"=>$param['ali_account'],"money"=>"-1"])->find();
                    //生成付款码备注
                    $note = M("order")->where(['account'=>$param['ali_account'],'code_id'=>$qrcode['code_id'],'addtime'=>['EGT',strtotime(date('Y-m-d'))]])->order('order_id DESC')->getField("note");
                    $uCode = $note + 1;
                    M("param")->where(" id = %d",$cParamId)->save(array("code_id"=>$qrcode["code_id"].'_'.$uCode));
                    $a = $astatus["ali_userid"];
                    $qrcode['qrdecode'] = 'alipays://platformapi/startapp?appId=09999988&actionType=toAccount&goBack=NO&amount='.$param['total_fee'].'&userId='.$a.'&memo='.$uCode;
                    //$qrcode['qrcode'] = 'alipays://platformapi/startapp?appId=09999988&actionType=toAccount&goBack=NO&amount='.$param['total_fee'].'&userId='.$a.'&memo='.$uCode;

                    //写订单
                    $param["note"] = $uCode;
                    $param["qrdecode"] = $qrcode["qrcode"];
                    $param["code_id"] = $qrcode["code_id"];
                    $param["account"] = $qrcode["account"];
                    ...
                    $row = M("qrcode_ali")->where($con)->getField('case when count(1) >= 100 then 1 else 0 end as c');
            ...
            M("param")->where(" id = %d",$cParamId)->save(array("return_time"=>date("Y-m-d H:i:s"),"bendi"=>$local,"data"=>json_encode($return_data)));
            keep_array($return_data);
...
//异步任务，支付结果
     ...
     * @return array
     */
    private function getQrcode($param=array()){
