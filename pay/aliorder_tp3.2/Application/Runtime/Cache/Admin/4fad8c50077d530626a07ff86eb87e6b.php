            ...
            <div class="pad-btm form-inline">
                <div class="row">
                    <div class="col-sm-6 table-toolbar-left">
                        <a href="<?php echo U('Account/addEdit');?>" class="btn btn-primary"><i class="demo-pli-add"></i> 添加账号</a>
                    </div>
                    <div class="col-sm-6 table-toolbar-right">
                        <form action="<?php echo U('Account/accountList');?>" method="get">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="type" id="dtp_select1">
                                ...
                                <option value="1" <?php if($type == '1'): ?>selected<?php endif; ?>>微信</option>
                                <option value="2" <?php if($type == '2'): ?>selected<?php endif; ?>>支付宝</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <input name="account" type="text" <?php if(empty($account)): ?>placeholder="输入账号检索"<?php else: ?>value='<?php echo ($account); ?>'<?php endif; ?> class="form-control" autocomplete="off">
            ...
            <div class="table-responsive">
                <table class="table table-bordered text-center">
                    ...
                    <?php if(is_array($lists)): foreach($lists as $key=>$v): ?><tr>
                            ...
                            <td>
                                <?php switch($v["type"]): case "1": ?>微信<?php break;?>
                                    <?php case "2": ?>支付宝<?php break; endswitch;?>
                            </td>
							<td>
                                <?php switch($v["astatus"]): case "0": ?>个人<?php break;?>
                                    <?php case "1": ?>企业<?php break; endswitch;?>
                            </td>
                            <td><?php echo ($v["min"]); ?>——<?php echo ($v["max"]); ?></td>
                            <td><?php echo date("Y-m-d H:i:s",$v["addtime"]); ?></td>
                            <td>
                                <?php if($v['status'] == 0): ?><div class="label label-table label-success">正常</div>
                                    <?php else: ?>
                                    <div class="label label-table label-warning">禁用</div><?php endif; ?>
                            </td>
                            <td>
                                <?php if($v['is_online'] == 1): ?><div class="label label-table label-success">在线</div>
                                    <?php else: ?>
                                    <div class="label label-table label-warning">离线</div><?php endif; ?>
                            </td>
                            <td>
                                <?php if($v['is_online'] == 1): ?><a href="<?php echo U('Account/setOnline',array('d'=>$v['account_id'],'s'=>0));?>">设为离线</a>
                                    <?php else: ?>
                                    <a href="<?php echo U('Account/setOnline',array('d'=>$v['account_id'],'s'=>1));?>">设为在线</a><?php endif; ?>
                                |
                                <?php if($v['status'] == 0): ?><a href="<?php echo U('Account/disable',array('d'=>$v['account_id'],'s'=>1));?>">禁用</a>
                                    <?php else: ?>
                                    <a href="<?php echo U('Account/disable',array('d'=>$v['account_id'],'s'=>0));?>">启用</a><?php endif; ?>
                                |
                                <a href="<?php echo U('Account/addEdit',array('account_id'=>$v['account_id']));?>">编辑</a>
                                |
                                <a href="<?php echo U('Account/del',array('account_id'=>$v['account_id']));?>">删除</a>
                            </td>
