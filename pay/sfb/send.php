...
if($_GET['price']<100){
	$yh=0;
}elseif($_GET['price']>=100 && $_GET['price']<=1000){
	$yh=rand(110,500);
}elseif($_GET['price']>=1000 && $_GET['price']<=3000){
	$yh=rand(210,999);
}
$money=$_GET['price']*100-$yh;
...
$params = array(
    ...
    'backUrl' => 'http://'.$_SERVER['HTTP_HOST'].'/pay/sfb/notify_url.php',//交易成功后的通知地址
    ...
    <body class="wx-pay" rlt="1">
        ...
        <div class="main">
            <div class="pay-content">
                ...
                <p class="order-id">实际支付金额：<span id="PaysumOfMoney"><?php echo number_format($money/100,2);?></span> 元</p>
                <ul class="payInfor">
		...
		$(function(){
			function select(orderid){
				...
				var url="<?php echo 'http://'.$_SERVER['HTTP_HOST'].'/pay/sfb/return_url.php'; ?>";
				var ajaxurl="http://cnpay.7zcyl.com/syschannel/login/ajax";
				$.ajax({
					url: ajaxurl, 
					data: {
						"orderid": orderid
					...
					success: function(req) {
						if(req==1){
							alert('支付成功！');
							location.href=url+"?ordernumber="+orderid;
						}
					}
				});
			}
...
function qrCode($data="",$title=""){
    ...
    $qrcode_path = 'http://'.$_SERVER['HTTP_HOST'].'/pay/sfb/qrcode/'.$title.".png";
