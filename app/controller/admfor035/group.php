...
class group extends CheckAdmin
{
    public function index()
    {
        ...
        $acl = $this->model()->select('a.gateway,b.name')->left('acw b')->on('a.acwid=b.id')->join()->from('acl a')
            ->where(array('fields' => 'a.acpcode=?', 'values' => array($acc['acpcode'])))->fetchAll();

//        $this->model()->select()->from('acp')->fetchAll();
//
//
//
...
//        $lists = $this->model()->select()->from('acc')->orderby('sortid asc,id desc')->fetchAll();
        ...
        $data = array('title' => '通道分组列表', 'lists' => $lists, 'acp' => $acp);
...
//	dump( $newData);
//
// $this->model()->from('acc')->updateSet($newData)->where(array('fields' => 'id=?', 'values' => array($id)))->update();
//        die();
...
//			echo "22222222222222";
    ...
    public function getAcl()
    {
        ...
        $str = '';
//        var_dump($this->model()->select('a.gateway,b.name')->left('acw b')->on('a.acwid=b.id')->join()->from('acl a')->where(array('fields' => 'a.acpcode=?', 'values' => array($acpcode)))->toDubugSql());
        if ($data) {
            foreach ($data as $key => $val) {
