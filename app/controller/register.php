...
class register extends Controller
{
    ...
    public function save()
    {
        ...
        if ($userinfo) {
            echo json_encode(array('status' => 0, 'msg' => '邮箱' . $email . '已被注册'));
            exit;
        }
        $superid = 0;
    ...
    public function savetwo()
    {
        ...
        if (!preg_match('/[0-9a-z]{5,20}/', $username)) {
            echo json_encode(array('status' => 0, 'msg' => '用户名格式错误，请使用5-20位小写字母或数字组合'));
            exit;
        }
        ...
        if (!($userid = $this->model()->from('users')->insertData($data)->insert())) {
            echo json_encode(array('status' => 0, 'msg' => '注册失败-err01'));
            exit;
        }
        ...
        if (!$this->model()->from('userinfo')->insertData($userinfo)->insert()) {
            ...
            echo json_encode(array('status' => 0, 'msg' => '注册失败-err02'));
            exit;
        }
